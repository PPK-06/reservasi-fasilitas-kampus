...
@section('page-actions')
    {{-- Tombol Tambah → A2 (belum dibuat, link aman saat route belum terdaftar) --}}
    <a href="{{ Route::has('admin.facilities.create') ? route('admin.facilities.create') : '#' }}"
       class="btn btn-primary btn-sm {{ Route::has('admin.facilities.create') ? '' : 'disabled' }}">
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body py-3">
        <form method="GET" action="{{ route('admin.facilities.index') }}" id="adminFilterForm">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-lg-3 col-md-12 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.facilities.index') }}" class="btn btn-outline-secondary btn-sm px-3">
...
{{-- ═══════════════════════════════════════════════════════
     ...
     Konvensi: card + table-responsive + thead biru primer
     (mengikuti UI snippets)
     ═══════════════════════════════════════════════════════ --}}
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
        <h6 class="mb-0 fw-bold text-dark">
            <i class="bi bi-building me-1"></i>Daftar Fasilitas
        </h6>
                ...
                <tbody>
                    @forelse ($facilities as $facility)
                        ...
                        <tr>
                            ...
                            {{-- Kapasitas (A4: tidak ditampilkan untuk tipe Alat) --}}
                            ...
                            <td class="text-end pe-3">
                                <div class="d-flex gap-1 justify-content-end">
                                    {{-- Edit → A2 (belum dibuat) --}}
                                    <a href="{{ Route::has('admin.facilities.edit') ? route('admin.facilities.edit', $facility) : '#' }}"
                                       class="btn btn-outline-primary btn-sm {{ Route::has('admin.facilities.edit') ? '' : 'disabled' }}"
                                       title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>

                                    {{-- Ubah status: active ↔ inactive (D3, admin only) --}}
                                    {{-- under_maintenance bukan wewenang admin di route ini --}}
                                    @if ($isActive)
                                        {{-- Aktif → Nonaktifkan --}}
                                        <button type="button"
                                                ...
                                                data-action="{{ route('admin.facilities.status', $facility) }}"
                                    ...
                                    @else
                                        {{-- under_maintenance: admin tidak bisa toggle,
                                             tombol dimatikan dengan tooltip penjelasan --}}
                                        <button type="button"
                                                class="btn btn-outline-secondary btn-sm"
                                                disabled
                                                title="Status 'Dalam Perbaikan' dikelola petugas">
                        ...
                        <tr>
                            <td colspan="7" class="text-center text-muted py-5">
                                ...
                                <p class="text-muted mb-0" style="font-size: 0.85rem;">
                                    ...
                                    <a href="{{ route('admin.facilities.index') }}" class="text-decoration-none">reset semua filter</a>.
                                </p>
                            </td>
                        </tr>
                    @endforelse
...
@foreach ($facilities as $facility)
    ...
    {{-- Hanya render modal untuk fasilitas yang punya tombol status --}}
            ...
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold" id="statusModalLabel-{{ $facility->id }}">
                        @if ($isActive)
                            <i class="bi bi-exclamation-triangle-fill text-danger me-2"></i>Nonaktifkan Fasilitas
                        @else
                            <i class="bi bi-check-circle-fill text-success me-2"></i>Aktifkan Fasilitas
                        @endif
                    </h5>
                ...
                <div class="modal-body pt-2">
                    @if ($isActive)
                        <p class="text-muted">
                            Kamu yakin ingin menonaktifkan fasilitas
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        {{-- Link ke O3 (officer.reservations.show)
                                                             Route ini milik M3 (Dhimas) — link aman
                                                             saat route belum terdaftar. --}}
                    ...
                    @else
                        <p class="text-muted mb-0">
                            Kamu yakin ingin mengaktifkan kembali fasilitas
                            <strong>{{ $facility->name }}</strong>?
                            Fasilitas akan dapat dipesan kembali oleh pengguna.
                        </p>
                    @endif
                </div>
                    ...
                    <form method="POST" action="{{ route('admin.facilities.status', $facility) }}">
                        ...
                        @if ($isActive)
                            <button type="submit" class="btn btn-danger">
                                <i class="bi bi-x-circle me-1"></i>Ya, Nonaktifkan
                            </button>
                        @else
